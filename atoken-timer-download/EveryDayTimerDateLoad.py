...
class OneTokenEveryDay(object):
    def __init__(self):
        ...
        self.exchange_list.append(self.huobi_list)
        self.exchange_list.append(self.binance_list)
        self.exchange_list.append(self.bittfinex_list)
        self.exchange_list.append(self.bittrex_list)
        self.exchange_list.append(self.bitz_list)
        self.exchange_list.append(self.gateio_list)
        self.exchange_list.append(self.gdax_list)
        self.exchange_list.append(self.hitbit_list)
        self.exchange_list.append(self.lbank_list)
        self.exchange_list.append(self.okex_list)
        self.exchange_list.append(self.kraken_list)
    ...
    def parse(self, url):
        ...
        for i in range(999):
            try:
                result = request_login.get(url, timeout=300)
    ...
    def depth_url_handle(self, date, currency):
        # 格式化下载页ｕｒｌ
    ...
    def data_load(self, url,path):
        # 下载历史数据压缩包
        for i in range(99999):
            time.sleep(2)
            try:
                response = request_login.get(url, timeout=300)
                ...
                file_name = './' + path + '/' + u_date + '.gz'
                size = sys.getsizeof(ret)
                print(str("%.3f" % (size / 1024)) + 'kb')
                if size / 1024 > 1:
    ...
    def run(self):
        # 获取列表的url可迭代对象
        import time
        datetime = time.strftime("%Y-%m-%d", time.localtime(time.time() - 86400*2))
